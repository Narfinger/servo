...
impl MainThreadWakerImpl {
    fn new(waker: Box<dyn EventLoopWaker>) -> Result<MainThreadWakerImpl, Error> {
        let callback =
            GenericCallback::new(move |_| waker.wake()).map_err(|_| Error::CommunicationError)?;
...
impl Registry {
    pub fn supports_session(&mut self, mode: SessionMode, dest: ProfileGenericCallback<Result<(), Error>>) {
    ...
    pub fn request_session(
        ...
        animation_frame_handler: GenericSender<Frame>,
    ) {
...
impl<GL: 'static + GLTypes> MainThreadRegistry<GL> {
    ...
    fn request_session(
        ...
        mode: SessionMode,
        init: SessionInit,
        raf_sender: GenericSender<Frame>,
...
enum RegistryMsg {
    RequestSession(
        SessionMode,
        SessionInit,
        GenericSender<Result<Session, Error>>,
        GenericSender<Frame>,
    ),
