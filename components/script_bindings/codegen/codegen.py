...
pub(crate) fn init_{name}_specs<D: DomTypes>() {{
    {name}_specs.set(Box::leak(Box::new([{joinedSpecs}])));
        ...
        joinedPrefableSpecs = ',\n'.join(prefableSpecs)
        prefArray = f"pub static {name}: ThreadUnsafeOnceLock<&[Guard<&[{specType}]>]> = ThreadUnsafeOnceLock::new();\n"
...
class CGDomObjectWrap(CGThing):
    ...
    def define(self) -> str:
        ...
        return f"""
            impl script_bindings::wrap::PrototypeID for {firstCap(string=ifaceName)} {{
                const ID_NUMBER: u16 = PrototypeList::ID::{MakeNativeName(self.descriptor.name)} as u16;
                const ID: PrototypeList::ID = PrototypeList::ID::{MakeNativeName(self.descriptor.name)};
                fn get_attribute_class() -> &'static [script_bindings::guard::Guard<&'static [js::jsapi::JSPropertySpec]>] {{
                    unsafe {{
                        crate::dom::bindings::codegen::GenericBindings::{toBindingPath(self.descriptor)}::sAttributes.get()
                    }}
                }}
                fn get_prototype_class() -> &'static js::jsapi::JSClass {{
                    &crate::dom::bindings::codegen::Bindings::{toBindingPath(self.descriptor)}::PrototypeClass
                }}
                fn get_class() -> &'static crate::dom::bindings::utils::DOMJSClass {{
                    unsafe {{
                        script_bindings::codegen::GenericBindings::{toBindingPath(self.descriptor)}::Class.get()
                    }}
                }}
        }}
        """
    ...
    def define(self) -> str:
        ...
        impl DomObjectIteratorWrap<crate::DomTypeHolder> for {name} {{
            unsafe fn ITER_WRAP(
                ...
                t: Box<IterableIterator<crate::DomTypeHolder, Self>>,
            ) -> Root<Dom<IterableIterator<crate::DomTypeHolder, Self>>> {{
                {bindingModule}::Wrap::<crate::DomTypeHolder>(cx,global,handle,t)
            }}
        }}
        """
...
class CGCreateInterfaceObjectsMethod(CGAbstractMethod):
    ...
    def definition_body(self) -> CGThing:
        ...
        if self.descriptor.interface.isNamespace():
            return CGGeneric(f"""
                use crate::wrap::{{CreateInterfaceObjectsOptions, CreateInterfaceObjectsRust, NamespaceInit }};
