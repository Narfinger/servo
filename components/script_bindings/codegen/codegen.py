...
def DomTypeHolder(descriptors: list[Descriptor],
    ...
    for descriptor in descriptors:
        if descriptor.interface.isCallback() or descriptor.interface.isIteratorInterface():
            continue
        if descriptor.interface.identifier.name == "Promise" or descriptor.interface.identifier.name == "PromiseNativeHandler":
            iface_name = descriptor.interface.identifier.name
            path = f"crate::{firstCap(iface_name)}"
...
class GlobalGenRoots():
    ...
    def InterfaceTypes(config: Configuration) -> CGThing:
        ...
        for name in descriptors:
            if name.lower()=="promise" or name.lower() == "promisenativehandler":
                pass
                # curr.append(CGGeneric(f"pub(crate) use script_bindings::{name.lower()}::{MakeNativeName(name)};\n"))
            else:
                curr.append(CGGeneric(f"pub(crate) use crate::dom::{name.lower()}::{MakeNativeName(name)};\n"))
        curr = CGList(curr)
