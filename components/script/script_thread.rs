...
pub struct ScriptThread {
    ...
    mutation_observer_microtask_queued: Rc<Cell<bool>>,
...
impl ScriptThread {
    ...
    pub(crate) fn mutation_observers() -> DomRefCell<Vec<Dom<MutationObserver>>> {
        with_script_thread(|script_thread| script_thread.mutation_observers.clone())
    }

    pub(crate) fn signals_and_slots_and_observers_and_queued() -> (
        DomRefCell<Vec<Dom<HTMLSlotElement>>>,
        DomRefCell<Vec<Dom<MutationObserver>>>,
        Rc<Cell<bool>>,
    ) {
        with_script_thread(|script_thread| {
            (
                script_thread.signal_slots.clone(),
                script_thread.mutation_observers.clone(),
                script_thread.mutation_observer_microtask_queued.clone(),
            )
        })
    }
